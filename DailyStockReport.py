...
input_source_csv = "Input/source-stocks.csv"
input_dest_csv = "Input/destination-stocks.csv"

output_daily_report = "Output-DailyReport/"
output_archived_report = "Output-ArchivedReport/"


allfiles = os.listdir(output_daily_report)
# iterate on all files to move them to destination folder
for f in allfiles:
    src_path = os.path.join(output_daily_report, f)
    dst_path = os.path.join(output_archived_report, f)
    shutil.move(src_path, dst_path)


df = pd.read_csv(input_source_csv)
dfp = df.iloc[np.random.randint(0, len(df), size=1)]

stock_url  = dfp['Screener Stock Url'].iloc[0]
stock_name = dfp['Screener Stock Symbol'].iloc[0]

xlsx_report_path = ScrapeSingleStock(stock_name,stock_url,output_daily_report)
html_report_path = VisualizeSingleStock(xlsx_report_path,output_daily_report)

dfp.head()

# Finally move the stocks for report is done into dest csv
df_done = pd.read_csv(input_dest_csv)
df_done = pd.concat([df_done, dfp],ignore_index=True, sort=False)
df_done.to_csv(input_dest_csv, index=False)

# remove that stock from source csv
df = df.drop( df[df['Screener Stock Symbol'] == stock_name].index  )
df.to_csv(input_source_csv, index=False)
